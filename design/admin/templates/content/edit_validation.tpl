{section show=$validation.processed}
    {section show=or( $validation.attributes, $validation.placement, $validation.business_rules )}
        <div class="message-warning">
            ...
            {section show=$validation.business_rules}
                <p>{'The following data is invalid according to the business rules'|i18n( 'design/admin/content/edit' )}:</p>
                <ul>
                    {section var=UnvalidatedBusinessRules loop=$validation.business_rules}
                        <li>{$UnvalidatedBusinessRules.item.text}</li>
                    {/section}
                </ul>
            {/section}
        </div>
